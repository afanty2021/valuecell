...
设置状态管理用户偏好设置，如股票颜色模式、语言和主题，并使用持久化存储。
...
useLanguage --> useSettingsStore : "使用"
...
**图示来源**
- [settings-store.ts](file://frontend/src/store/settings-store.ts#L1-L144)
...
股票详情页面集成了TradingView高级图表组件，显示股票的K线图和其他详细信息。

```mermaid
flowchart TD
Stock[股票详情页面] --> BackButton[返回按钮]
Stock --> Chart[交易图表]
Stock --> Info[股票信息]
Chart --> TradingView[TradingView高级图表]
Info --> Business[业务摘要]
Info --> Properties[属性信息]
TradingView --> ThemeSync[主题同步]
TradingView --> LocaleSync[语言同步]
```

**图示来源**
- [stock.tsx](file://frontend/src/app/home/stock.tsx#L1-L146)
...
```mermaid
flowchart TD
TradingView[TradingView高级图表] --> Props[属性配置]
Props --> Ticker[股票代码]
Props --> Interval[时间间隔]
Props --> Theme[主题]
Props --> Locale[语言]
Props --> Timezone[时区]
TradingView --> Script[动态脚本]
Script --> Embed[嵌入式小部件]
Embed --> Chart[图表渲染]
Chart --> Copyright[版权信息]
```

**图示来源**
- [tradingview-advanced-chart.tsx](file://frontend/src/components/tradingview/tradingview-advanced-chart.tsx#L1-L144)
...
5. **国际化集成**：在设置状态中集成语言切换功能，并自动更新i18n实例
6. **主题同步**：将应用主题与第三方组件（如TradingView）的主题保持同步
...
5. **状态持久化**：使用Tauri的store插件实现本地状态持久化
6. **更新机制**：利用Tauri的更新插件实现应用自动更新

### 国际化(i18n)实现

应用使用i18next实现国际化支持，支持多种语言。

```mermaid
flowchart TD
i18n[i18n实例] --> Resources[资源文件]
Resources --> en[英文资源]
Resources --> zhCN[简体中文资源]
Resources --> zhTW[繁体中文资源]
Resources --> ja[日文资源]
i18n --> Settings[设置存储]
Settings --> Language[语言选择]
Language --> i18n : "changeLanguage"
Settings --> useSettingsStore : "setLanguage"
```
...
- [i18n/index.ts](file://frontend/src/i18n/index.ts#L1-L34)
- [settings-store.ts](file://frontend/src/store/settings-store.ts#L1-L144)
- [zh_CN.json](file://frontend/src/i18n/locales/zh_CN.json)
...
```mermaid
flowchart TD
Theme[主题管理] --> nextThemes[Next Themes]
nextThemes --> useTheme : "useTheme()"
Theme --> CSS[CSS变量]
CSS --> :root[根元素变量]
CSS --> .dark[深色类变量]
Theme --> TradingView[TradingView同步]
TradingView --> themeProp[主题属性传递]
TradingView --> resolvedTheme[解析主题]
```

**图示来源**
- [global.css](file://frontend/src/global.css#L1-L194)
- [general.tsx](file://frontend/src/app/setting/general.tsx#L1-L185)
